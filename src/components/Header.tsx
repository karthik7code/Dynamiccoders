...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [langCategory, setLangCategory] = useState<'all' | 'scheduled' | 'regional'>('all');
  const [showToolsMenu, setShowToolsMenu] = useState(false);
  const [showAccountMenu, setShowAccountMenu] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const baseLanguages =
    langCategory === 'scheduled'
      ? SCHEDULED_INDIAN_LANGUAGES
      : langCategory === 'regional'
      ? REGIONAL_INDIAN_LANGUAGES
      : ALL_INDIAN_LANGUAGES;

  const filteredLanguages = baseLanguages.filter(
    (l) =>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-3">
        ...
        <nav className="hidden xl:flex items-center gap-1 font-bold text-xs">
          ...
          >
            ...
            <span>{t('recommender', selectedLang)}</span>
          </button>
          ...
          >
            <span>{t('formguide', selectedLang)}</span>
          </button>
            ...
            {showToolsMenu && (
              <div className="absolute left-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in">
                ...
                >
                  ...
                  <span>{t('database', selectedLang)}</span>
                </button>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className="p-2 rounded-full hover:bg-slate-100 text-slate-700 flex items-center gap-1.5 text-xs font-bold border border-slate-200"
              title="Select Indian Language"
            >
              <Globe className="w-4 h-4 text-amber-600" />
              <span className="hidden sm:inline font-bold">
                {getLanguageByCode(selectedLang)?.nativeName || 'English'}
              </span>
            </button>

            {showLangMenu && (
              <div className="absolute right-0 mt-2 w-72 bg-white border border-slate-200 rounded-2xl shadow-xl p-2.5 z-50 animate-in fade-in">
                {/* Header: Strictly Indian Languages */}
                <div className="px-2 py-1 flex items-center justify-between border-b border-slate-100 mb-2 pb-2">
                  <div className="flex items-center gap-1.5">
                    <span className="text-sm">🇮🇳</span>
                    <span className="text-[11px] font-extrabold text-[#00003c] uppercase tracking-wider">
                      Only Indian Languages
                    </span>
                  </div>
                  <span className="text-[9px] bg-amber-100 text-amber-900 font-extrabold px-2 py-0.5 rounded-full">
                    {ALL_INDIAN_LANGUAGES.length} Bharatiya Bhashas
                  </span>
                </div>

                {/* Reset to English (Original Default) */}
                <div className="mb-2">
                  <button
                    onClick={() => {
                      setSelectedLang('en');
                      ...
                      setLangSearch('');
                      showToast({
                        title: 'Default View Restored',
                        description: 'Viewing portal in default English.',
                        type: 'info',
                      });
                      applyPageTranslation('en', true);
                    }}
                    className={`w-full text-left px-2.5 py-1.5 rounded-xl text-xs flex items-center justify-between transition-colors ${
                      selectedLang === 'en'
                        ? 'font-extrabold text-[#00003c] bg-slate-100 border border-slate-300'
                        : 'text-slate-600 hover:bg-slate-50 border border-dashed border-slate-200'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-xs">🌐</span>
                      <span className="font-semibold">Reset to English (Original)</span>
                    </div>
                    {selectedLang === 'en' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0 ml-1" />}
                  </button>
                </div>

                {/* Category Filter Tabs */}
                <div className="flex items-center gap-1 mb-2 bg-slate-100 p-1 rounded-xl">
                  <button
                    onClick={() => setLangCategory('all')}
                    className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-colors ${
                      langCategory === 'all'
                        ? 'bg-white text-[#00003c] shadow-xs'
                        : 'text-slate-600 hover:text-slate-900'
                    }`}
                  >
                    All ({ALL_INDIAN_LANGUAGES.length})
                  </button>
                  <button
                    onClick={() => setLangCategory('scheduled')}
                    className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-colors ${
                      langCategory === 'scheduled'
                        ? 'bg-white text-[#00003c] shadow-xs'
                        : 'text-slate-600 hover:text-slate-900'
                  ...
                  <button
                    onClick={() => setLangCategory('regional')}
                    className={`flex-1 py-1 rounded-lg text-[10px] font-bold transition-colors ${
                      langCategory === 'regional'
                        ? 'bg-white text-[#00003c] shadow-xs'
                        : 'text-slate-600 hover:text-slate-900'
                ...
                <div className="relative mb-2">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-2 py-1 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:ring-1 focus:ring-[#00003c]"
                  />
                </div>

                {/* Only Indian Languages List */}
                <div className="max-h-60 overflow-y-auto space-y-0.5 pr-1">
                  {filteredLanguages.length === 0 ? (
                    <p className="text-xs text-slate-400 p-3 text-center">No Indian language found matching search</p>
                  ) : (
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          showToast({
                            ...
                            description: `Switched language to ${lang.nativeName} (${lang.name}). Translating 100% of portal...`,
                            type: 'info',
                          });
                          applyPageTranslation(lang.code, true);
                        }}
